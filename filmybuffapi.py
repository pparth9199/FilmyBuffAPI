...
def ProfileAPI():
    ...
    try:
        if abase64:
            ...
            cur.execute("insert into usert(first_name,last_name,screen_name,u_password,marital_s,prof,gmail_id,avatar_id,favgenre,region) values (\""+fname+"\",\""+lname+"\",\""+screenname+"\",\""+str(password)+"\",\""+str(mstatus)+"\",\""+prof+"\",\""+email+"\",\""+str(aid)+"\",\""+str(sgenre)+"\",\""+str(sregion)+"\",\""+password+"\");")
...
def viewResult():
    ...
    try:
        ...
        for x in ic:
            if x==True:
                cur.execute("INSERT INTO user_questions (user_id,qid) VALUES("+str(userid)+",\""+qid[k]+"\");")
                cur.execute("COMMIT")
            k=k+1
        
        cur.execute("select u_coins,correct_images,u_level,wrong_images from achievements WHERE user_id=\""+str(userid)+"\";")
        row_headers = [x[0] for x in cur.description]  # this will extract row headers
        myresult = cur.fetchall()
        for i in myresult:
        ...
        cur.execute("COMMIT")
        cur.execute("select u_level as \"level\" from achievements where user_id=\""+str(userid)+"\"")
        myresult=cur.fetchall()
        row_headers = [x[0] for x in cur.description]
        res=[]
        for result in myresult:
